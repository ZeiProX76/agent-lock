...
- **On Windows, five of the twenty-two tests do not run.** CI is green on ubuntu, macOS and Windows across Node 18, 20 and 22, but the four tests that stand in for a coding agent with a `/bin/sh` script and the one that drives a real pty skip there and say so. So the gate, the launch chain and the checker's isolation are proven on macOS and Linux and only reasoned about on Windows; the inventory, the flags, the semantic diff, the seal and the platform helpers are proven everywhere.
