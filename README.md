...
- **Windows runs the gate in the middle, not in front.** Windows has no `exec`, so the `.cmd` and `.ps1` shims hand the launch to `agent-lock launch`, which decides and then runs the tool itself, forwarding the console and the exit code. One extra process in the tree, and Ctrl-C reaches the tool the same way.
...
- **On Windows the pty test does not run, and one test is not applicable.** The stand-in tool the suite launches is a Node script behind a `.cmd`, so the gate, the launch chain, the exit code and the checker's isolation are exercised on Windows the same as everywhere else, and the `.cmd` command line agent-lock builds is parsed by a real `cmd.exe`. Two things stay outside that: the menu inside a real pty (no ConPTY driver in the suite) and the POSIX `/bin/sh` shim, which does not exist on Windows and has its own `.cmd` equivalent test.
- **Arguments you type yourself are not promised byte for byte through a `.cmd`.** `claude` and `codex` ship as `.exe` on Windows and never take that path; an npm-installed CLI like `gemini` does, and `cmd.exe` re-reads the command line, so a `%VAR%` or a `^` in an argument can arrive changed. What agent-lock builds itself (flags, a model name, a path, an empty argument) round-trips exactly, and CI asserts it against a real `cmd.exe`. What is asserted for the rest is that an argument never splits in two or disappears.
