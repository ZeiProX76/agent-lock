...
- **On Windows, 27 of the 30 tests run.** The stand-in tool the suite launches is a Node script behind a `.cmd`, so the gate, the launch chain, the exit code and the checker's isolation are exercised there the same as everywhere else, and the `.cmd` command line agent-lock builds is parsed by a real `cmd.exe`. Three skip and say why: the POSIX `/bin/sh` shim, which does not exist on Windows and has its own `.cmd` equivalent test; a POSIX file-permission test; and the menu inside a real pty, which the suite drives with `python3` and has no ConPTY equivalent for.
