...
export async function launch(tool, args) {
  ...
  // A .cmd shim puts its arguments back on a command line for cmd.exe to read a second time.
  // One shape does not survive that intact, and guessing at it is how an extra command gets run.
  const unsafe = run.options.windowsVerbatimArguments ? unsafeForCmd(all) : [];
  if (unsafe.length) {
    out(
      red(
        `agent-lock: ${tool} is a .cmd shim, and cmd.exe reads its command line twice, so ${JSON.stringify(unsafe[0])} cannot be passed through unchanged.`
      ),
