...
// The one shape that survives the first parse and breaks the second. `%*` puts the arguments back
// on a command line and cmd reads them again; there a literal quote closes the quoted region that
// everything after it was relying on, and a `&` or a `|` sitting outside quotes is read as the
// next command. Neither half is a problem alone, so a quoted prompt stays allowed and a path with
// brackets stays allowed. Together they are the injection, and both halves are named.
const CHAINS = /[&|<>]/;
export function unsafeForCmd(args) {
  const quoted = (a) => a.includes('"');
  if (!args.some(quoted) || !args.some((a) => CHAINS.test(a))) return [];
  return args.filter((a) => quoted(a) || CHAINS.test(a));
}
