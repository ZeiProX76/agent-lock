...
export function printInventory(inv, { trusted = [], entry = null } = {}) {
  ...
  const files = [...inv.files].sort((a, b) => Number(isHotKind(b.kind)) - Number(isHotKind(a.kind)) || a.rel.localeCompare(b.rel));
  for (const f of files.slice(0, 60)) {
    const mark = isHotKind(f.kind) ? cyan('●') : dim('○');
    const desc = describe(inv, f);
    out(`   ${mark} ${isHotKind(f.kind) ? f.rel : dim(f.rel)}  ${dim(kb(f.size))}${desc ? dim(`  ${desc}`) : ''}`);
  }
  if (files.length > 60) out(dim(`   … ${files.length - 60} more files (docs, skills), all hashed`));
