...
import { addWindowsPath, cmdShim, ps1Shim, removeWindowsPath } from './windows.mjs';
...
function writeShims(tools) {
  ...
  for (const t of tools) {
    ...
    if (IS_WINDOWS) {
      ...
      fs.writeFileSync(path.join(BIN_DIR, `${t}.ps1`), ps1Shim(t));
    }
  }
  ...
  if (IS_WINDOWS) {
    ...
    fs.writeFileSync(
      path.join(BIN_DIR, 'agent-lock.ps1'),
      `#!/usr/bin/env pwsh\r\n& "${process.execPath}" "${MJS}" @args\r\nexit $LASTEXITCODE\r\n`
    );
  }
}
...
export async function install({ strict = false } = {}) {
  ...
  writeShims(tools);
  const where = IS_WINDOWS
    ? addWindowsPath()
    : `PATH + alias lines added to ${addPathLine().join(', ') || '(already present)'}`;
  out(
  ...
  out(
    ...
    dim(
      '   claude plugin marketplace add <owner>/agent-lock && claude plugin install agent-lock   (ConfigChange backstop)'
    )
  );
...
export function uninstall() {
  ...
  if (IS_WINDOWS) removeWindowsPath();
