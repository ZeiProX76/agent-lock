...
function configFlags(file, isHome) {
  ...
  for (const [name, s] of Object.entries(servers)) {
    ...
    if (m && (!m[2] || /^@(latest|next)$/.test(m[2]))) flags.push(`${rel}: MCP server "${name}" runs npx ${m[1]}${m[2] || ''} with no version pin (whatever the registry serves at launch)`);
  }
  return flags;
}
...
function commandFlags(inv) {
  ...
  for (const c of inv.commands) {
    ...
    for (const ref of referencedPaths(c.command, inv.root)) {
      ...
      if (EDITOR.has(own) !== EDITOR.has(target)) flags.push(`${c.file}: "${c.command}" runs a file inside ${target}/ (${own} → ${target} cross-reference, the August 2026 keyv worm shape)`);
  ...
  for (const [key, n] of shared) { const [file, target] = key.split('|'); flags.push(`${file}: ${n} command${n > 1 ? 's' : ''} run files inside ${target}/ (hook scripts shared between tools; read them once)`); }
  return flags;
}
